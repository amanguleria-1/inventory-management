...
class BillClass:
    def __init__(self, root):
        ...
        btn_check = Button(customerFrame, text="Check", font=(
            "times new roman", 15, "bold"), bg="lightblue", cursor="hand2", borderwidth=0).place(x=470, y=30, width=50, height=30)
        ...
        lbl_delivery_charges = Label(customerFrame, text="Delivery Charges:", font=(
            "SF Pro Rounded", 15), bg="white").place(x=5, y=235)
        txt_care_of = Entry(customerFrame, bd=2, text=self.var_del_chrg, font=(
            "SF Pro Rounded", 13), bg="lightgray").place(x=130, y=235, width=160, height=22)
        ...
        self.cartTable = ttk.Treeview(cart_Frame, columns=(
            "pid", "name", "price", "qty", "Amount"), yscrollcommand=scrolly.set, xscrollcommand=scrollx.set)
        scrollx.pack(side=BOTTOM, fill=X)
        scrolly.pack(side=RIGHT, fill=Y)
        ...
        btn_clear_cart = Button(add_cart_widget_Frame, text="Clear", font=(
            "SF Pro Rounded", 15, "bold"), relief=RIDGE, command=self.clear_cart, bg="black", fg="white", cursor="hand2").place(x=150, y=90, height=20, width=150)
        btn_add_cart = Button(add_cart_widget_Frame, text="Add | Update", command=self.add_update_cart, font=(
            "SF Pro Rounded", 15, "bold"), relief=RIDGE, bg="black", fg="white", cursor="hand2").place(x=150, y=110, height=20, width=150)
...
# ===
        ...
        try:
            if self.var_contact.get() == "":
                messagebox.showerror(
                    "Error", "Please Enter Mobile No.", parent=self.root)
            else:
                cur.execute("select * from customer where " +
                            self.var_contact.get()+" LIKE '%"+self.var_contact.get()+"%'")
    ...
    def bill_updates(self):
        ...
        #         "Error", "Please mention delivery charges(Type 0 for Free Delivery)", parent=self.root)
        #     return
        self.del_charge = int(self.var_del_chrg.get())
        for row in self.cart_list:
