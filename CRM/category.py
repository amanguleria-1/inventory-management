...
class categoryClass:
    def __init__(self, root):
        ...
        # ---------Title--------
        lbl_title = Label(self.root, text="Manage Product Category", font=(
        ...
        lbl_name = Label(self.root, text="Enter Category Name", font=(
            "SF Pro Rounded", 30, "bold"), bg='white').place(x=50, y=100)
        txt_name = Entry(self.root, textvariable=self.var_name, font=(
            "SF Pro Rounded", 18, "bold"), bg='white').place(x=50, y=170, width=300)
        btn_add = Button(self.root, text="ADD", command=self.add, font=(
            "SF Pro Rounded", 15, "bold"), bg='white', cursor="hand2").place(x=360, y=170, width=150, height=30)
        btn_delete = Button(self.root, text="DELETE", command=self.delete, font=(
            "SF Pro Rounded", 15, "bold"), bg='white', cursor="hand2").place(x=520, y=170, width=150, height=30)

        # Category Details
        cat_frame = Frame(self.root, bd=3, relief=RIDGE)
        cat_frame.place(x=700, y=100,  width=380, height=180)
        ...
        try:
            if self.var_name.get() == "":
                ...
                if row != None:
                    ...
                    con.commit()
                    messagebox.showinfo(
                        "Success", "Category added succesfully", parent=self.root)
                    self.show()
    ...
    def delete(self):
        ...
        try:
            if self.var_cat_id.get() == "":
                messagebox.showerror(
                    "Error", "Category ID or name is required", parent=self.root)
            else:
                ...
                if row == None:
                    ...
                    if op == True:
                        ...
                        con.commit()
                        messagebox.showinfo(
                            "Delete", "Category deleted sucessfully", parent=self.root)
                        self.show()
                        self.var_cat_id.set("")
                        self.var_name.set("")
        except Exception as ex:
